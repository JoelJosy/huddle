...
for select
to authenticated
using ((bucket_id = 'note-contents'::text));
...
for insert
to authenticated
with check ((bucket_id = 'note-contents'::text));
...
for delete
to authenticated
using (((bucket_id = 'note-contents'::text) AND ((storage.foldername(name))[1] = (auth.uid())::text)));
